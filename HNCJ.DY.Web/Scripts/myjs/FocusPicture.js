...
function initTable(data) {
    $('#tt').datagrid({
        url: '/FocusPicture/GetAllFocusPictures',
        title: '用户信息列表',
        //iconCls: 'icon-save',
        width: 700,
        height: 400,
        ...
        columns: [[
            ...
            { field: 'ID', title: '主键', width: 40 },
            { field: 'Title', title: '标语', width: 300 },
            { field: 'ImagePath', title: '焦点图', width: 60, formatter: function (value, row, index) { return "<img src='" + value + "' width='20px'height='20px' />"; } },
            { field: 'Url', title: '焦点图URL', width: 120 },
            { field: 'RegTime', title: '时间', width: 120, align: 'right', formatter: function (value, row, index) { return (eval(value.replace(/\/Date\((\d+)\)\//gi, "new Date($1)"))).pattern("yyyy-M-d h:m:s"); } }
        , {
            field: 'ModfiedOn', title: '操作 ', width: 80, formatter: function (value, row, index) {
                var str = "";
...
function editEvent(id) {
    ...
    $("#editframe").attr("src", "/FocusPicture/Edit/" + id);
...
function deleteEvent(ids) {
    ...
    //alert(ids);
    $.post("/FocusPicture/Delete", { ids: ids }, function (data) {
        if (data == "ok") {
            initTable();
        } else {
...
function addClickEvent() {
    $("#addframe").attr("src", "/FocusPicture/Add");
