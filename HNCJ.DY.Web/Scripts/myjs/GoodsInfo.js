...
function initTable(data) {
    $('#tt').datagrid({
        url: '/GoodsInfo/GetAllGoodsInfos',
        title: '用户信息列表',
        //iconCls: 'icon-save',
        width: 700,
        height: 400,
        ...
        columns: [[
            ...
            { field: 'GoodsPrice', title: '价格(元)', width: 120 },
            { field: 'GoodsNum', title: '数量', width: 120 },
            { field: 'GoodsRemark', title: '备注', width: 120 },
...
function editEvent(id) {
    ...
    $("#editframe").attr("src", "/GoodsInfo/Edit/" + id);
...
function deleteEvent(ids) {
    ...
    //alert(ids);
    $.post("/GoodsInfo/Delete", { ids: ids }, function (data) {
        if (data == "ok") {
            initTable();
        } else {
...
function addClickEvent() {
    $("#addframe").attr("src", "/GoodsInfo/Add");
