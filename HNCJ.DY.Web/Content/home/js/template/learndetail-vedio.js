...
function learnRight() {
    ...
    $.post("/LearnData/GetAllVedios", { type: 1 }, function (data) {
        var learn = data;
        for (var i = 0; i < learn.length ; i++) {
            var myLearn = '';
            myLearn = '<div class="right-title"><a href="learndetail-video.html?id='+learn[i].ID+'">' + learn[i].Name + ' </a></div>';
...
function talk(page,id) {
    ...
    if (page == "") page = 1;
    $.post("/LearnData/GetContentInfos", { key: id, page: page, rows: 10 }, function (data) {
        var talk = data.datas;
        $("#other-talking").empty();
        $("#pag").empty();
        for (var i = 0; i < talk.length ; i++) {
            var myTalk = '';
            myTalk = '<div id="other-talk"><img src="' + talk[i].Icon + '" alt=""> <div class="talk-content-bottom"> <p class="talk-one"></p> <div class="talk-content">' + talk[i].dContent + '</div> </div></div>';
...
function talkcontent() {
    var id = Request("id");
    $.post("/LearnData/GetContentInfos", { key: id }, function (data) {
...
function Click() {
    ...
    if(content=="")return;
    $.post("/LearnData/AddContent", { id: id, content: content }, function (data) {
        if (data.status == 1) {
            talk("", id);
        } else {
            alert("出错了");
...
function OnClick() {
    var id = Request("id");
    $.post("/LearnData/ClickZan2", { id: id }, function (data) {
        if (data.status == 1) {
            $("#zan").empty();
